...
// ChatRequest is the request body sent to the Groq API.
type ChatRequest struct {
	Model    string    `json:"model"`
	Messages []Message `json:"messages"`
}

// ChatResponse represents the full response from Groq.
...
// Usage holds token usage info from the Groq response.
type Usage struct {
...
// GroqProvider implements the provider.Provider interface for Groq.
type GroqProvider struct{}

// New creates a new GroqProvider.
func New() *GroqProvider {
	return &GroqProvider{}
}
...
// Execute performs the actual HTTP call to the Groq API.
// Includes automatic retry with key rotation on 429 errors.
func (g *GroqProvider) Execute(payload []byte, model string) provider.Result {
	...
	// Build the Groq-specific request
	reqBody := ChatRequest{
		Model:    model,
	...
	return provider.Result{
		...
		ProviderName:    "groq",
		ModelUsed:       model,
		Error:           fmt.Errorf("all Groq API keys are rate-limited"),
	}
}

// ValidateKey makes a minimal API call to verify a Groq API key works.
func (g *GroqProvider) ValidateKey(apiKeyValue string) error {
